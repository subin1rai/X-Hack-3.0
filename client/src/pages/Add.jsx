...
const AddPlantForm = ({ showForm, setShowForm, fetchVegDetails }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post(`${baseUrl}/api/plants/add`, formDataToSend, {
        headers: {
      ...
      <div className="bg-white rounded-2xl max-w-3xl w-full p-8 relative max-h-[90vh] overflow-y-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-500 mt-1">
            Enter the details of your plant product
          </p>
        </div>
          ...
          <div>
            ...
            <div className="border-2 border-dashed border-gray-200 rounded-xl p-6 hover:border-[#2D775C]/50 transition-colors">
              ...
              <p className="text-sm text-gray-500 mt-2">
                Upload one or more images of your plant
              </p>
            </div>
          </div>
...
const Add = () => {
  ...
  const fetchVegDetails = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await axios.get(`${baseUrl}/api/plants/farmer-plants`, {
        headers: {
          Authorization: `Bearer ${token}`,
          "ngrok-skip-browser-warning": "69420",
        },
      ...
      setError(
        e.response?.data?.ErrorMessage || "Failed to fetch vegetable details."
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleDelete = async (id) => {
    try {
      const token = localStorage.getItem("token");
      await axios.delete(`${baseUrl}/api/plants/${id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
